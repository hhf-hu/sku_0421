...
import json
from transformers import AutoProcessor, AutoModel  # 修改为 AutoProcessor 和 AutoModel
import random
...
class DFN5BTestImageCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            # 使用 SigLIP 处理器处理图像和文本
            inputs = self.processor(
                text=[caption],  # 修改为列表形式
                images=image,
                return_tensors="pt",
                padding=True,
...
def test_model(use_main_category_only=False):
    ...
    output_dir = f"siglip_evaluation_results_character_{label_type}"  # 修改输出目录名称
