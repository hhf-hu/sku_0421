...
class DFN5BTestImageCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"警告：无法打开图片文件 {image_path}: {e}，将跳过此样本。")
            return None
...
def test_model(csv_path, use_main_category_only=True):
    """主测试函数 - 图片到图片检索"""
    # 配置参数
    ...
    test_df = generate_dataset_info_from_csv(csv_path, use_main_category_only=use_main_category_only)

    print(f"测试集大小: {len(test_df)}")

    # 创建数据集和数据加载器
    test_dataset = DFN5BTestImageCaptionDataset(test_df, processor)
    ...
    print(f"标签类型: {'主类别' if use_main_category_only else '子类别'}")
    print(f"{'=' * 60}")
...
if __name__ == "__main__":
    ...
    print("测试使用完整标签...")
    print("=" * 50)
    test_model(CSV_PATH, use_main_category_only=False)

    # # 测试仅使用主类别
    # print("\n" + "=" * 50)
    # print("测试仅使用主类别...")
    # print("=" * 50)
    # test_model(CSV_PATH, use_main_category_only=True)