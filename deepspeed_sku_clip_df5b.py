...
def captions_to_labels(captions):
    """
    Convert a list of caption strings to a tensor of integer labels,
    where identical captions get the same label.
    """
    unique_captions = {}
    labels = []
    next_label = 0
    for cap in captions:
        if cap not in unique_captions:
            unique_captions[cap] = next_label
            next_label += 1
        labels.append(unique_captions[cap])
    return torch.tensor(labels, dtype=torch.long)

def margin_ce_loss(logits, label, margin=0.2):
    # 将对角线（正样本）的logits减去margin
    margin_logits = logits.clone()
    margin_logits[torch.arange(logits.size(0)), label] -= margin
    return torch.nn.functional.cross_entropy(margin_logits, label)
...
if __name__ == "__main__":
    ...
    # --- 配置参数 ---
    character_path = "/Users/vincent/workspace/trademark_similar/character_data"#"/data1/vincent/datasets/item_Winnie_data1211/" #"/Users/vincent/workspace/trademark_similar/character_data"#"/data1/vincent/datasets/data_gray/"
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        for batch in progress_bar:
            ...
            # if config.text_config.max_position_embeddings != 77:
            #     print("max_position_embeddings:", config.text_config.max_position_embeddings)
            #     print("检测到配置问题，尝试修复...")
            #     config.text_config.max_position_embeddings = 77
            # if model.text_model.config.max_position_embeddings !=77:
            #     print("max_position_embeddings:", model.text_model.config.max_position_embeddings)
            #     print("检测到配置问题，尝试修复...")
            #     model.text_model.config.max_position_embeddings = 77
            ...
            if True:
                # Modify the labels using captions: TODO
                # labels_img = captions_to_labels(captions).to(device)
                seen = set()
                keep_indices = []
                ...
                keep_indices = torch.tensor(keep_indices, dtype=torch.long).to(logits_img_per_img.device)

                # Step 2: Filter logits matrix to only kept rows AND columns
                logits_filtered = logits_img_per_img[keep_indices][:, keep_indices]  # (K, K), K = num unique

                # Step 3: Create standard diagonal labels
                labels_filtered = torch.arange(logits_filtered.size(0), device=logits_img_per_img.device)

            loss_i = margin_ce_loss(logits_per_image, labels)
            loss_t = margin_ce_loss(logits_per_text, labels)
            loss_img = margin_ce_loss(logits_img_per_img,labels)
            # loss = (loss_i + loss_t) / 2
            lmd = 1
            # 修改/opt/anaconda3/envs/sglang/lib/python3.10/site-packages/transformers/models/clip/modeling_clip.py的CLIPModel的forward
            # logits_img_per_img = self.logit_scale.exp()*torch.matmul(image_embeds,image_embeds.t().to(text_embeds.device)).to(text_embeds.device)
