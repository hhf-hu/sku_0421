...
def evaluate(model, dataloader, device, world_size=1):
    ...
    if world_size > 1:
        ...
        dist.all_gather_object(all_similarity_matrices_gathered, full_similarity_matrix.numpy())
        dist.all_gather_object(all_cosine_similarities_gathered, all_cosine_similarities)

        if dist.get_rank() == 0:  # 只在主进程计算指标
            all_preds_combined = []
            all_labels_combined = []
